...
export default function RelatorioTI() {
  ...
  const [invisibleSpaces, setInvisibleSpaces] = useState(17);
  const [spacingPanelExpanded, setSpacingPanelExpanded] = useState(false);
  ...
  const gerarPDF = async () => {
    ...
    try {
      ...
      // Criar uma cópia do relatório para adicionar espaços invisíveis apenas no PDF
      const relatorioClone = relatorioRef.current.cloneNode(true) as HTMLElement;
      
      // Remover o indicador visual de espaços invisíveis da cópia
      const visualIndicator = relatorioClone.querySelector('.noPrint');
      if (visualIndicator) {
        visualIndicator.remove();
      }
      
      // Adicionar espaços invisíveis antes do footer apenas na cópia
      if (invisibleSpaces > 0) {
        const footer = relatorioClone.querySelector(`.${styles.footer}`);
        if (footer) {
          ...
          footer.parentNode?.insertBefore(invisibleSpaceDiv, footer);
        }
      }
      
      // Posicionar a cópia fora da tela para captura
      relatorioClone.style.position = 'absolute';
      relatorioClone.style.left = '-9999px';
      relatorioClone.style.top = '0';
      document.body.appendChild(relatorioClone);
      ...
      // Remover a cópia do DOM
      document.body.removeChild(relatorioClone);
  ...
  return (
    <div style={{ background: "linear-gradient(135deg, #F4F6FA 0%, #E8E8E8 100%)", minHeight: "100vh", padding: 20 }}>
      {/* Painel esquerdo - Controles de espaçamento e gerenciador */}
      <div style={{ position: "fixed", top: 20, left: 20, zIndex: 1000, display: "flex", flexDirection: "column", gap: 16, alignItems: "flex-start" }}>
        {/* Controles de espaçamento invisível com expansão/recolhimento */}
        <div style={{
          background: 'white',
          borderRadius: '12px',
          padding: spacingPanelExpanded ? '20px' : '12px',
          boxShadow: '0 4px 12px rgba(0,0,0,0.1)',
          border: '1px solid #e0e0e0',
          minWidth: spacingPanelExpanded ? '280px' : 'auto',
          transition: 'all 0.3s ease'
        }}>
          <div style={{ 
            display: 'flex', 
            alignItems: 'center', 
            justifyContent: spacingPanelExpanded ? 'space-between' : 'center',
            cursor: 'pointer',
            marginBottom: spacingPanelExpanded ? '12px' : '0'
          }} onClick={() => setSpacingPanelExpanded(!spacingPanelExpanded)}>
            <h3 style={{ 
              color: '#4a90e2', 
              margin: '0', 
              fontSize: spacingPanelExpanded ? '16px' : '14px',
              fontWeight: '600'
            }}>
              {spacingPanelExpanded ? 'Controle de Espaçamento' : `Espaços: ${invisibleSpaces}`}
            </h3>
            <span style={{
              fontSize: '18px',
              color: '#4a90e2',
              transform: spacingPanelExpanded ? 'rotate(180deg)' : 'rotate(0deg)',
          ...
          {spacingPanelExpanded && (
            <>
              <p style={{ margin: '0 0 16px 0', color: '#666', fontSize: '14px' }}>
                Ajuste a posição do footer no relatório
              </p>
      ...
      {/* Painel direito - Gerar PDF e Guia */}
          ...
          <div className={styles.reportInfo}>
            <div className={styles.infoBlock}>
              <div className={styles.infoRow}>
                ...
                <span className={styles.infoValue} contentEditable suppressContentEditableWarning>Janeiro - julho 2025</span>
              </div>
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>Responsável:</span>
                <span className={styles.infoValue} contentEditable suppressContentEditableWarning>Hiago dos Santos Barbosa</span>
              </div>
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>Data do Relatório:</span>
                <span className={styles.infoValue} contentEditable suppressContentEditableWarning>31 de Julho, 2025</span>
              </div>
              <div className={styles.infoRow}>
                <span className={styles.infoLabel}>Cargo:</span>
                <span className={styles.infoValue} contentEditable suppressContentEditableWarning>Desenvolvedor de Sistemas</span>
              </div>
            </div>
          </div>
            ...
            <div className={styles.activityGrid}>
              {atividades.map((a, i) => (
                <div className={`${styles.activityCard} ${i === 1 ? styles.activityCardHighlight : ''}`} key={i}>
                  {/* Estrela de destaque para layout padrão */}
                  {i === 1 && (
                    <img 
                      src="/star.png"
                      alt="Destaque"
        ...
        <footer className={styles.footer}>
          <div className={styles.signature}>
            <div className={styles.signatureBlock}>
              ...
              <div className={styles.signatureLabel} contentEditable suppressContentEditableWarning>Thiago Holanda Carneiro<br/>Gerente de Ti</div>
            </div>
            <div className={styles.signatureBlock}>
              <div className={styles.signatureLine}></div>
              <div className={styles.signatureLabel} contentEditable suppressContentEditableWarning>Manoel Messias da Silva-T.Cel<br/>Presidente</div>
            </div>
          </div>
          <div>
          <svg
          ...
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <div className={styles.guiaSecao}>
                ...
                <ul>
                  <li><strong>Controle de Espaçamento:</strong> Clique para expandir/recolher o painel</li>
                  <li><strong>+ Espaço:</strong> Adiciona espaço invisível para empurrar o footer</li>
                  <li><strong>- Espaço:</strong> Remove espaço invisível</li>
                  <li><strong>Visualização:</strong> Os espaços aparecem apenas na tela, não no PDF</li>
                </ul>
              </div>
